...
# get LD for parental alleles
ld <- genetics::LD(go_df)

ld_D <- t(ld[[2]])
ld_Dprime <- t(ld[[3]])
mean_LD_Dprime <- mean(ld_Dprime, na.rm = T) # 0.0089
sd_LD_Dprime <- sd(ld_Dprime, na.rm = T) # 0.0516

ld_R2 <- t(ld[[5]])
mean_LD_R2 <- mean(ld_R2, na.rm = T) # 0.0089
sd_LD_R2 <- sd(ld_R2, na.rm = T) # 0.0516


#------------------------------------------------------------------------------#
# Use mutant alleles
#------------------------------------------------------------------------------#
test <- data.table::fread("data/processed/02_DFE_unimputed_genotypes.csv")
gm2 <- data.table::fread("data/processed/02_DFE_unimputed_genotypes.csv") %>%
  ...
  dplyr::ungroup() %>%
  dplyr::select(-n.na) %>%
  dplyr::mutate(genotype = case_when(genotype == 1 ~ "M/M",
                                     genotype == 0 ~ "W/W",
                                     TRUE ~ NA_character_)) %>%
  dplyr::select(-MA530, -MA563) %>%
  tidyr::pivot_wider(names_from = "strain", values_from = "genotype")
...
go_list2 <- NULL
for (i in 1:nrow(gm2)) {
  go_list2[[i]] <- genetics::genotype(as.character(gm2[i, 2:ncol(gm2)]))
}
...
# get LD measures for mutant alleles
ld2 <- genetics::LD(go_df2)


ld2_D <- t(ld2[[2]])
ld2_Dprime <- t(ld2[[3]])
mean_LD2_Dprime <- mean(ld2_Dprime, na.rm = T) # 0.0089
min_LD2_Dprime <- min(ld2_Dprime, na.rm = T) # 0.0089
sd_LD2_Dprime <- sd(ld2_Dprime, na.rm = T) # 0.0516

ld2_R2 <- t(ld2[[5]])
mean_LD2_R2 <- mean(ld2_R2, na.rm = T) # 0.0089
sd_LD2_R2 <- sd(ld2_R2, na.rm = T) # 0.0516
#------------------------------------------------------------------------------
view(ld_Dprime)
view(ld2_Dprime)
view(ld2_D)
...
strains <- data.table::fread("data/processed/02_DFE_unimputed_genotypes.csv") %>%
  dplyr::mutate(snp_id = paste0(chrom, "_", pos)) %>%
  dplyr::select(snp_id, everything(), -chrom:-N2_G0_ANC) %>%
  tidyr::pivot_longer(cols = -snp_id:-MA563, names_to = "strain", values_to = "genotype") %>%
  dplyr::group_by(strain) %>%
...
#Genetic distance
#Physical position
map <- data.table::fread("data/processed/02_DFE_unimputed_genotypes.csv") %>%
...
write_tsv(map, file = "data/processed/dfe_all.map", col_names = F)
...
ped[ped==100] <- "0"
ped[is.na(ped)] <- "0\t0"
write_tsv(ped, file = "data/processed/dfe_all.ped", col_names = F)

# make the snp-list
snp_list <- map$full_var_id
write_file(paste(snp_list, collapse = " "), file = "data/processed/snps.txt")

# run plink - commands run on QUEST
#plink --file dfe --allow-extra-chr --ld-window-r2 0 --r inter-chr dprime-signed --ld-snp-list snps.txt --out mut_allele_ld_r
#plink --file dfe --allow-extra-chr --ld-window-r2 0 --r2 inter-chr --ld-snp-list snps.txt --out mut_allele_ld_r2

# load in the plink dprime_signed table
dpri <- data.table::fread("data/processed/mut_allele_ld_r.ld") %>%
  dplyr::select(SNP_A, SNP_B, DP) %>%
  tidyr::pivot_wider(names_from = SNP_B, values_from = DP) %>%
  tibble::column_to_rownames("SNP_A")
dpri[upper.tri(dpri, diag=TRUE)] <- NA
dpri <- as.matrix(dpri)

r2 <- data.table::fread("data/processed/mut_allele_ld_r2.ld")  %>%
  dplyr::select(SNP_A, SNP_B, R2) %>%
  tidyr::pivot_wider(names_from = SNP_B, values_from = R2) %>%
  tibble::column_to_rownames("SNP_A")
r2[upper.tri(r2, diag=TRUE)] <- NA
r2 <- as.matrix(r2) # set as matrix

# campare plink and genetics values
test <- r2 - ld2_R2
test2 <- as_tibble(test) %>%
  dplyr::mutate(var_id = rownames(test), .before = 1) %>%
  tidyr::pivot_longer(cols = -var_id) %>%
  dplyr::filter(!is.na(value))
r2_diff_hist <- ggplot(test2) +
  geom_jitter(aes(x = 1, y = value)) +
  geom_violin(aes(x = 1, y = value))
r2_diff_hist
# The r2 values for variants near V_1692283_del are MUCH larger than the ld2_R2
mean_r2 <- mean(r2, na.rm = T) # 0.0089
sd_r2 <- sd(r2, na.rm = T) # 0.0516

#==============================================================================#
# END Test PLINK for CB
#==============================================================================#
#=========================================================
